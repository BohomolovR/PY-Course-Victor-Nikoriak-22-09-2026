...
> ⚠️ **Поточний стан**: `requirements.txt` і launcher-скрипти (`install_course.*`/`start_course.*`) ще не перенесені в цей (новий) репозиторій — це заплановано на одну з наступних фаз міграції. Інструкція нижче описує загальний процес; крок 4 (встановлення бібліотек курсу) стане доступним, як тільки `requirements.txt` з'явиться в репозиторії.
